...
import { fermerConversation } from "@/lib/services/conversations";
...
export const POST = routeApi<Ctx>(async (_req, { params }) => {
  ...
  const conversation = await fermerConversation(id);
  if (!conversation) return reponseErreur("Conversation introuvable", { status: 404 });

  return reponseOk(null, { message: "Conversation fermée" });
});
